...
import time
import ssl
from config.config import (get_cohere_api_key, get_pinecone_api_key, PINECONE_INDEX_NAME)
...
def get_clients():
    try:
        ...
        st.error(f"Failed to initialize API clients. Please check your API keys and network connection. Error: {e}")
        st.stop()
...
def setup_vector_store(chunks, cohere_client, pinecone_client):
    ...
    # Check if the index is empty before embedding
    if index.describe_index_stats()['total_vector_count'] == 0:
        with st.spinner("Embedding... This is a one-time setup."):
            batch_size = 96
            total_batches = (len(chunks) + batch_size - 1) // batch_size
            
            progress_text = "Embedding. Please wait."
            ...
            for i in range(0, len(chunks), batch_size):
                ...
                try:
                    ...
                    progress_percentage = min(current_batch_num / total_batches, 1.0)
                    my_bar.progress(progress_percentage, text=f"Processed batch {current_batch_num}/{total_batches}")
